...
## 12. Esconder/mostrar itens com `hidden`/`display:none` pode "desfazer" reveals de ScrollTrigger já tocados nos itens vizinhos
**Bug real (gastronomia, filtro do cardápio):** cada card do grid tinha `[data-reveal]` com `scrollTrigger:{...toggleActions:"play none none reverse"}`. Ao clicar num chip de filtro e esconder 7 dos 9 itens via `item.hidden=true`, o grid encolheu de altura — o reflow resultante fez os 2 itens que continuavam visíveis (já com opacity:1 há muito tempo) voltarem para opacity:0, porque o ScrollTrigger reage à mudança de layout como se o elemento tivesse saído da viewport e dispara o "reverse" do toggleActions.
**Regra:** depois de qualquer show/hide em massa que mude a altura da página (filtro, acordeão, tabs), se os itens afetados (ou vizinhos) tiverem reveal via ScrollTrigger com `reverse` no toggleActions, force `gsap.set(item, {opacity:1, y:0})` nos itens que devem ficar visíveis E chame `ScrollTrigger.refresh()` logo depois — não confie que o estado já revelado vai "ficar" só porque a animação já tocou uma vez.
